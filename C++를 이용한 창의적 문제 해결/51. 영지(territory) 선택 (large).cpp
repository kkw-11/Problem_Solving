...
// }
 ...
 //int main() {
 //	//freopen("input.txt", "rt", stdin);
 //	int H, W, HH, HW, maxCount=0, count;
 //	int** map;
 //	int** totalCountMap;

 //	scanf("%d %d", &H, &W);
 //	map = (int**)malloc(sizeof(int*) * (H + 1));
 //	totalCountMap = (int**)malloc(sizeof(int*) * (H + 1));

 //	for (int i = 0; i < (H + 1); ++i) {
 //		map[i] = (int*)calloc(W + 1, sizeof(int));
 //		totalCountMap[i] = (int*)calloc(W + 1, sizeof(int));
 ...
 //		for (int j = 1; j <= W; ++j) {
 //			scanf("%d", &map[i][j]);
 //			totalCountMap[i][j] = totalCountMap[i - 1][j] + totalCountMap[i][j - 1] - totalCountMap[i - 1][j - 1]+ map[i][j];
 //		}
 //	}
 //	scanf("%d %d", &HH, &HW);
 ...
 //	for (int k = HH; k <= H; ++k) {
 //		for (int l = HW; l <= W; ++l) {
 //			count = totalCountMap[k][l] - totalCountMap[k][l-HW] - totalCountMap[k-HH][l] + totalCountMap[k-HH][l-HW];
 //			if (count > maxCount)
 //				maxCount = count;
 //		}
