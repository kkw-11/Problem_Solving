def solution(num_teams, remote_tasks, office_tasks, employees):
    ...
    # 출근하는 팀원이 아무도 없는 팀은 앞번호가 제거되어야 하므로 재택근무자 추려내기 위한 알고리즘전에 정렬
    sub_home_member.sort()

    # 재택근무자 후보중에 진짜 재택근무자 추려내기
    ## 재택 근무자 후보중에서 제외되어야 할 사람은 아무도 출근하지 않는 경우이다.
    ## 위에서 구한 출근하는 팀번호에 우리팀이 있는지 체크해서 없으면 우리팀 번호를 출근팀에 넣고 나는 재택근무자에서 빠진다.(여기서 빠진다는 answer 값에 들어가지 않는다.는 의미이다.)
